...
interface CompanyProfileScreenProps extends NativeStackScreenProps<BaseStackParamList, "Main"> {}
    ...
    return (
      <Screen style={themed($root)} preset="scroll">
        <View style={themed($headerContainer)}>
          ...
          >
            <Icon icon="settings" size={30} color={theme.colors.palette.angry500} />
          </Pressable>
        </View>

        <View style={themed($profileContainer)}>
          <TouchableOpacity onPress={pickImage} style={themed($logoContainer)}>
            {companyStore.profilePhoto ? (
              <Image source={{ uri: companyStore.profilePhoto }} style={themed($profileImage)} />
            ) : (
              <Icon icon="settings" size={40} color={theme.colors.palette.accent500} />
            )}
          </TouchableOpacity>
          <View style={themed($profileInfoContainer)}>
            ...
            />
            <Text text={companyStore.email || "Немає email"} preset="formHelper" />
          </View>
        </View>
        ...
        <View style={themed($buttonContainer)}>
          <Button text="Logout" onPress={() => authStore.logout()} style={themed($logoutButton)} />
        </View>
      </Screen>
    )
  },
